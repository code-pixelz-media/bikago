 ...
 * admin-specific hooks, and public-facing site hooks.
 */
//require plugin_dir_path( __FILE__ ) . 'public/partials/bikago-shop-manager-defaults-2.php';
...
function bikago_get_image_id($image_url) {
  ...
  $flex = new Bikago_Shop_Manager_Flexiroam();
    $sim = $flex->availableSim();

    $qr_code = $sim['data'][0]["qr_code"];
...
function set_form(){
    ...
    // $body = kyc_email_template( $order_id, $plan, $customer_name );
    $approval = 'verified';

    if( !class_exists( 'Bikago_Shop_Manager_Flexiroam' )){
    include (BIKAGO_SHOP_MANAGER_PUBLIC . 'includes/class-bikago-shop-manager-flexiroam.php');
    }
    ...
    $sku = $sim['data'][0]["sku"];
    $qr_code = $sim['data'][0]["qr_code"];
    $product_code = $sim['data'][0]["product_code"];

     // $plan_code = $plan
    $flexiroam_plan = get_post_meta($plan_id, 'plan-flexiroam', true);
    $flex->purchasePlanExistingSim( $sku, $flexiroam_plan ); 

    $iccid = $flex->getIccid( $sku ); // '2019092965961'

    $order_no = $flex->getOrderNO( $sku ); // '2019092965961'


    $body =  verification_email_template_flexiroam( $order_id, $plan, $approval,  $qr_code ); 
...
//global$TN20220804192243B36F5D74" 
  //$flex = new Bikago_Shop_Manager_Flexiroam();
    // $sim = $flex->availableSim();
    ...
    // //  $plan_code = $plan
    // // $flexiroam_plan = get_post_meta($plan, 'plan-flexiroam', true);
    // // I-250MB-7D-ID
    // //$flexiroam_plan = "I-250MB-7D-ID";
    // $flexiroam_plan = "I-1GB-7D-ID";
    // // $plans = $flex->planInventory(); 

    // //var_dump( $plans->data);
    // // foreach($plans->data as $plan){
    // //     var_dump($plan->plan->plan_code);
    // // }

    // // var_dump( $plan );

    // var_dump( $flex->purchasePlanExistingSim( '3022104805450', "I-1GB-7D-ID" ) ); 
// $sku = 3022104805452;
//     $iccid = $flex->getIccid( $sku ); // '2019092965961'
